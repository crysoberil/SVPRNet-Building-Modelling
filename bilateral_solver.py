# Adapted from: Ben Poole
# GitHub Source: https://github.com/poolio/bilateral_solver
...
_RGB_TO_YUV_ = np.array([[0.299, 0.587, 0.114], [-0.168736, -0.331264, 0.5], [0.5, -0.418688, -0.081312]], dtype=np.float32)
_YUV_TO_RGB_ = np.array([[1.0, 0.0, 1.402], [1.0, -0.34414, -0.71414], [1.0, 1.772, 0.0]], dtype=np.float32)
_YUV_OFFSET_ = np.array([0, 128.0, 128.0], dtype=np.float32)[np.newaxis, np.newaxis, :]


def rgb2yuv(im):
    return np.tensordot(im, _RGB_TO_YUV_, ([2], [1])) + _YUV_OFFSET_


def yuv2rgb(im):
    return np.tensordot(im.astype(float) - _YUV_OFFSET_, _YUV_TO_RGB_, ([2], [1]))
...
class BilateralGrid(object):
    def __init__(self, im, grid_params=_default_grid_params_):
        sigma_spatial = grid_params["sigma_spatial"]
        sigma_luma = grid_params["sigma_luma"]
        sigma_chroma = grid_params["sigma_chroma"]
        im_yuv = rgb2yuv(im)
        ...
        self.hash_vec = (_MAX_VAL_ ** np.arange(self.dim))
...
class BilateralSolver(object):
    def __init__(self, grid, params=_bs_params_):
        self.grid = grid
        self.params = params
